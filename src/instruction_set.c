...
uint8_t register_to_modrm(register_t reg) {
    switch (reg) {
        case REG_AL: case REG_AX: return 0;
        case REG_CL: case REG_CX: return 1;
        case REG_DL: case REG_DX: return 2;
        case REG_BL: case REG_BX: return 3;
        case REG_AH: case REG_SP: return 4;
        case REG_CH: case REG_BP: return 5;
        case REG_DH: case REG_SI: return 6;
        case REG_BH: case REG_DI: return 7;
        // Segment registers
        case REG_ES: return 0;
        case REG_CS: return 1;
        case REG_SS: return 2;
        case REG_DS: return 3;
        default: return 0;
    }
}
...
int get_register_size(register_t reg) {
    switch (reg) {
        ...
        case REG_CS: case REG_DS: case REG_ES: case REG_SS:
            return 16;
        default:
            return 0;
    }
...
uint8_t register_to_rm16(register_t reg) {
    ...
    switch (reg) {
        case REG_BX: return 7;  // [bx]
        case REG_BP: return 6;  // [bp+disp] (note: [bp] alone is actually [bp+0])
        case REG_SI: return 4;  // [si]
        case REG_DI: return 5;  // [di]
        // Note: 16-bit addressing has limited base register options
        default: return 6; // Default to BP addressing
    }
}
...
static uint8_t get_base_index_rm(register_t base, register_t index) {
    ...
    if (base == REG_BX && index == REG_SI) return 0; // [bx+si]
    if (base == REG_BX && index == REG_DI) return 1; // [bx+di]
    if (base == REG_BP && index == REG_SI) return 2; // [bp+si]
    if (base == REG_BP && index == REG_DI) return 3; // [bp+di]
...
static bool encode_inc_dec(const instruction_t* instr, uint8_t* buffer, size_t* size, assembler_t* asm_ctx) {
    ...
    if (instr->operands[0].type == OPERAND_REGISTER) {
        ...
        if (instr->operands[0].value.memory.has_label) {
            ...
            *size = 4;        } else if (instr->operands[0].value.memory.index != REG_NONE) {
        ...
        if (instr->operand_count == 2) {
            ...
            if (instr->operands[0].type == OPERAND_REGISTER && 
                ...
                result = encode_mov_reg_imm(&temp_instr, temp_buffer, &temp_size);
                       ...
                       instr->operands[1].type == OPERAND_MEMORY) {
                result = encode_mov_reg_mem(instr, temp_buffer, &temp_size);            } else if (instr->operands[0].type == OPERAND_MEMORY && 
                       instr->operands[1].type == OPERAND_REGISTER) {
